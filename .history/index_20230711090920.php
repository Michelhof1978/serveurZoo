                ...
                switch($url[1]){//On va vérifier la valeur du 2 ème champs (position 1 ds tableau) après le / ds l url
                   ...
                   case "animaux": $apiController -> getAnimaux();//création route nommé animaux, on récupére les infos ds API.controller.php grâce à la fonction getAnimaux
                        if(!isset($url[2])) || (!isset($url[3])){
                            // Code à exécuter si $url[2] est en position 2 de l'url ou $url[3] en position 3 n'est pas défini
                            $apiController -> getAnimaux(-1,-1);
                        }else{ //Si les positions 2 et 3 de l'URL sont définies, le bloc de code suivant sera exécuté.
                            $apiController -> getAnimaux((int)$url[2],(int)$url[3]);//Les url sont automatiquement en string donc ici, on va les convertir
                        }
