...
class APIManager extends Model{//Système d héritage avec extends
    public function getDBAnimaux($idFamille, $idcontinent){//L'utilisation du modificateur public permet de rendre la 
            $whereClause = "";//méthode accessible et utilisable de manière externe à la classe, ce qui est souvent
                 if ($idFamille !== -1 || $idcontinent !== -1) //Est ce que famille à au moins une valeur
                     $whereClause .= " WHERE ";//$whereClause .= " WHERE " : Cela signifie que la variable $whereClause est concaténée avec la chaîne " WHERE ".
                     if ($idFamille !== -1)  $whereClause .= " f.famille_id = :idFamille ";//$whereClause .= " f.famille_id = :idFamille " : Cela signifie que la variable $whereClause est concaténée avec la chaîne " f.famille_id = :idFamille ".
                        if ($idcontinent !== -1) //Est ce que continent à au moins une valeur
                            if ($idFamille !== -1) $whereClause .= " AND ";//$whereClause .= " AND " : Cela signifie que la variable $whereClause est concaténée avec la chaîne " AND ".
                            $whereClause .= " c.continent_id = :idContinent ";//$whereClause .= " c.continent_id = :idContinent " : Cela signifie que la variable $whereClause est concaténée avec la chaîne " c.continent_id = :idContinent ".
