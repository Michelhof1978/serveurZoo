...
class APIManager extends Model{//Système d héritage avec extends
    public function getDBAnimaux(){
      ...
      "inner join famille f on f.famille_id = a.famille_id" : Effectue une jointure interne (INNER JOIN) avec la table "famille" en utilisant la clé étrangère "famille_id" présente dans la table "animal".
      "inner join animal_continent ac on ac.animal_id = a.animal_id" : Effectue une jointure interne (INNER JOIN) avec la table "animal_continent" en utilisant la clé étrangère "animal_id" présente dans la table "animal".
      "inner join continent c on c.continent_id = ac.continent_id" : Effectue une jointure interne (INNER JOIN) avec la table "continent" en utilisant la clé étrangère "continent_id" présente dans la table "animal_continent".
      En résumé, cette requête sélectionne toutes les colonnes de la table "animal" et effectue des jointures avec les tables "famille", "animal_continent" et "continent" en utilisant les clés étrangères appropriées. Cela permet d'obtenir les informations liées aux animaux, à leur famille et aux continents auxquels ils appartiennent.
