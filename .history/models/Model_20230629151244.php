...
abstract class Model {//En héritant de la classe abstraite "Model", les classes enfants peuvent étendre ou modifier le comportement
    ...
    public static function sendJSON($info){//Transformer les datas en format JSON pour API
        ...
        header("content-Type: application/json");
                         La ligne de code "header("content-Type: application/json");" en français signifie que l'en-tête de la réponse HTTP va indiquer que le contenu de la réponse est de type JSON. Voici une explication de chaque terme :

            - "header" : C'est une fonction en PHP utilisée pour envoyer des en-têtes HTTP. Les en-têtes HTTP fournissent des informations supplémentaires sur la réponse que le serveur envoie au client. Dans ce cas, la fonction "header" est utilisée pour définir l'en-tête "Content-Type".
            
            - "content-Type" : C'est un en-tête HTTP qui spécifie le type de contenu de la réponse. Il indique comment le contenu de la réponse doit être interprété par le client. Dans ce cas, "content-Type" est utilisé pour spécifier que le contenu de la réponse est de type JSON.
